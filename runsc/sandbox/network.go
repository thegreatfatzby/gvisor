...
func getLoopbackArgs(conf *config.Config) boot.CreateLinksAndRoutesArgs {
...
func getNSArgs(nsPath string, conf *config.Config) (boot.CreateLinksAndRoutesArgs, error) {
	args := boot.CreateLinksAndRoutesArgs{}
	...
	if err != nil {
		return args, fmt.Errorf("querying interfaces: %w", err)
	}
	...
	if isRoot {
		return args, fmt.Errorf("cannot run with network enabled in root network namespace")
	}
	...
	for _, iface := range ifaces {
		...
		if err != nil {
			return args, fmt.Errorf("fetching interface addresses for %q: %w", iface.Name, err)
		}
		...
		if iface.Flags&net.FlagLoopback != 0 {
			...
			if err != nil {
				return args, fmt.Errorf("getting loopback link for iface %q: %w", iface.Name, err)
			}
		...
		for _, ifaddr := range allAddrs {
			...
			if !ok {
				return args, fmt.Errorf("address is not IPNet: %+v", ifaddr)
			}
		...
		if err != nil {
			return args, fmt.Errorf("fetching ARP table for %q: %w", iface.Name, err)
		}
		...
		if err != nil {
			return args, fmt.Errorf("getting routes for interface %q: %v", iface.Name, err)
		}
		if defv4 != nil {
			if !args.Defaultv4Gateway.Route.Empty() {
				return args, fmt.Errorf("more than one default route found, interface: %v, route: %v, default route: %+v", iface.Name, defv4, args.Defaultv4Gateway)
			}
		...
		if defv6 != nil {
			if !args.Defaultv6Gateway.Route.Empty() {
				return args, fmt.Errorf("more than one default route found, interface: %v, route: %v, default route: %+v", iface.Name, defv6, args.Defaultv6Gateway)
			}
		...
		if err != nil {
			return args, fmt.Errorf("getting link for interface %q: %w", iface.Name, err)
		}
		...
		for _, addr := range ipAddrs {
			...
			if err := removeAddress(ifaceLink, addr.String()); err != nil {
				...
				if present, err := isAddressOnInterface(iface.Name, addr); err != nil {
					return args, fmt.Errorf("checking if address %v is on interface %q: %w", addr, iface.Name, err)
				} else if !present {
					continue
				}
				return args, fmt.Errorf("removing address %v from device %q: %w", addr, iface.Name, err)
			}
		}
		...
		if conf.XDP.Mode == config.XDPModeNS {
			...
			if err != nil {
				return args, fmt.Errorf("failed to create XDP socket: %v", err)
			}
			...
			for i := 0; i < link.NumChannels; i++ {
				...
				if err != nil {
					return args, fmt.Errorf("failed to createSocket for %s : %w", iface.Name, err)
				}
				if i == 0 {
					link.GSOMaxSize = socketEntry.gsoMaxSize
				} else {
					if link.GSOMaxSize != socketEntry.gsoMaxSize {
						return args, fmt.Errorf("inconsistent gsoMaxSize %d and %d when creating multiple channels for same interface: %s",
...
func createInterfacesAndRoutesFromNS(conn *urpc.Client, nsPath string, conf *config.Config) error {
	...
	args, err := getNSArgs(nsPath, conf)
	if err != nil {
		return err
	}
...
func setupRestoreNetworkArgs(pid int, conf *config.Config) (boot.CreateLinksAndRoutesArgs, error) {
	log.Infof("Restore network args")
	...
	default:
		return boot.CreateLinksAndRoutesArgs{}, fmt.Errorf("invalid network type during restore: %v", conf.Network)
	}
}
