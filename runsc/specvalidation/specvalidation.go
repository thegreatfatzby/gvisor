...
// Package specvalidation provides options for spec validation during restore.
...
// Handle deals with spec validation according to the given policy.
func Handle(oldSpecs, newSpecs map[string]*specs.Spec, conf *config.Config) error {
	...
	case config.RestoreSpecValidationWarning:
		...
		if err := validateSpecs(oldSpecs, newSpecs); err != nil {
			log.Warningf("failed to validate restore spec: %w", err)
		}
